...
from mods import DEFAULT_MODS, get_mods, is_modified, save
...
def _pick_guild(session, guild_id):
    for g in _available_guilds(session):
...
async def _apply(session, guild_id, data):
    guild = _pick_guild(session, guild_id)
    if guild is None:
        return {"ok": False, "error": "No server is available. Make sure the bot is in the server and you have Manage Server."}
    if data.get("reset"):
        ...
        payload = data.get("mods") or {}
        save(BOT, guild.id, payload)
    mods = get_mods(BOT, guild.id)
    return {
        "ok": True,
        "mods": mods,
...
<title>BirdBot Panel</title><style>
...
<h2>BirdBot Panel</h2><p>{message}</p><p><a href="/">Back home</a></p>
    ...
    def _handle_bootstrap(self, query):
        ...
        try:
            payload = _run_on_loop(_gather(session, guild_id))
